...
class PluginDeactivatedEvent extends AbstractEvent
{
    /** @return string */
    public function getName()
    {
        return __("Plugin deactivated", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    {
        return __("Whenever a plugin is deactivated, send a notification.", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    {
        return __("%user_name% *deactivated plugin* %plugin_name% on site <%site_url%|%site_title%> at %date_time%.", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    public function register()
    {
        ...
        add_action('deactivated_plugin', function($plugin, $networkDeactivating)
        {
            ...
            foreach($this->getIntegrations() as $integration)
            {
                ...
                $attachment->fields[] = new SlackFieldModel(
                    __('Deactivated by', ConfigService::TEXT_DOMAIN_NAME),
