...
class PostUpdatedEvent extends AbstractPostEvent
{
    /** @return string */
    public function getName()
    {
        return __("Post updated", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    {
        return __("Whenever a post is updated, send a notification.", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    {
        return __("%user_name% *updated %post_type%* <%post_url%|%post_title%> on site <%site_url%|%site_title%> at %date_time%.", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    public function register()
    {
        add_action('post_updated', function($postId, $postAfter, $postBefore){
            ...
            foreach($this->getIntegrations() as $integration)
            {
                ...
                $slackMessageModel->text = $this->renderMessage($integration, $postId, $postAfter);
                $slackMessageModel->attachments[] = $this->getAttachment($integration, $postId, $postAfter);
