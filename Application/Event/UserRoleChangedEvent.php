...
class UserRoleChangedEvent extends AbstractEvent
{
    /** @return string */
    public function getName()
    {
        return __("User role changed", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    {
        return __("Whenever a user role changes, send a notification.", ConfigService::TEXT_DOMAIN_NAME);
    }
    ...
    {
        return __("*%user_name%* his *system roles changed* on site <%site_url%|%site_title%> at %date_time%.", ConfigService::TEXT_DOMAIN_NAME);
    }
        ...
        add_action('set_user_role', function($userId, $role, $oldRoles){
            ...
            foreach($this->getIntegrations() as $integration)
            {
                ...
                $slackMessageModel->text = strtr($this->getMessage($integration), [
                    '%user_role_previous%' => implode(', ', $previousRoles),
                    '%user_role_current%' => implode(', ', $currentRoles),
                ...
                $attachment->fields[] = new SlackFieldModel(
                    __('Previous role(s)', ConfigService::TEXT_DOMAIN_NAME),
